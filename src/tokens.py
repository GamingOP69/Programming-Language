...
class TokenType(Enum):
    ...
    LAMBDA = auto()         # lambda keyword
    YIELD = auto()          # yield keyword (for generators)
    ...
    AMPERSAND = auto()      # &
    PIPE = auto()           # |
    PIPE_PIPE = auto()      # |> (pipe operator)
    CARET = auto()          # ^
    TILDE = auto()          # ~
    LESS_LESS = auto()      # <<
    GREATER_GREATER = auto()  # >>
    DOT = auto()            # .
    DOT_DOT_DOT = auto()    # ... (spread/rest)
    COMMA = auto()          # ,
    COLON = auto()          # :
    SEMICOLON = auto()      # ;
    ARROW = auto()          # ->
    LAMBDA_ARROW = auto()   # => (lambda arrow)
    DOT_DOT = auto()        # .. (range)
    QUESTION = auto()       # ? (ternary)
    ELVIS = auto()         # ?? (null coalescing)
